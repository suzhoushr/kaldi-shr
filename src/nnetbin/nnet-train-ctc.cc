...
//    po.Register("use-gpu", &use_gpu, "yes|no|optional, only has effect if compiled with CUDA"); 
    ...
    while (1) {
        ...
        net.ResetLstmSeq(new_utt_flags);
        ...
        // Backward pass
        if (!crossvalidate) {
            CuMatrix<BaseFloat> trans_diff;
            trans_diff.Resize(obj_diff.NumRows(), obj_diff.NumCols(), kSetZero);
            for (int s = 0; s < cur_sequence_num; s++) {
                for (int r = 0; r < frame_num_utt[s]; r++) {
                    int index = r*cur_sequence_num + s;
                    trans_diff.Row(index).CopyFromVec(obj_diff.Row(index));
                }
            }

            net.Backpropagate(trans_diff, NULL);
        ...
        if (total_frames == 0) {
            ...
            KALDI_VLOG(1) << net.InfoPropagate();
            if (!crossvalidate) {
                KALDI_VLOG(1) << net.InfoBackPropagate();
                KALDI_VLOG(1) << net.InfoGradient();
            }
        }
        /*// VERBOSE LOG
        ...
        if (kaldi::g_kaldi_verbose_level >= 2) {
            ...
            if (counter >= 50000) {
                ...
                KALDI_VLOG(2) << net.InfoPropagate();
                if (!crossvalidate) {
                    KALDI_VLOG(2) << nnt.InfoBackPropagate();
                    KALDI_VLOG(2) << net.InfoGradient();
                }
                counter = 0;
            }
        }*/
