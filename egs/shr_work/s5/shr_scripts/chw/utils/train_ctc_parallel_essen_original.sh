...
halving_after_epoch=12    # halving bcomes enabled after this many epochs

# logging
report_step=10          # during training, the step (number of utterances) of reporting objective and accuracy
verbose=1
...
if [ -z $nnet_init ]; then 
	if [ ! -z $nnet_proto ]; then
		if [ ! -f $dir/nnet/nnet.iter0 ]; then
			...
			net-initialize --binary=true $nnet_proto $dir/nnet/nnet.iter0 >& $dir/log/initialize_model.log || exit 1;
		fi
	fi
fi
