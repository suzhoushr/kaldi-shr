...
train_tool=nnet-train-ctc  # the command for training; by default, we use the
...
if $sort_by_len; then
  ...
  paste -d " " $data_tr/feats.scp $dir/len.tmp | sort -k3 -n - | awk '{if($3<=110) print $1 " " $2}' | sort -R > $dir/train.scp || exit 1;
  cp $dir/len.tmp $dir/len.tr
  feat-to-len scp:$data_cv/feats.scp ark,t:- | awk '{print $2}' > $dir/len.tmp || exit 1;
  paste -d " " $data_cv/feats.scp $dir/len.tmp | sort -k3 -n - | awk '{if($3<=110) print $1 " " $2}' | sort -R > $dir/cv.scp || exit 1;
...
if [ -z $nnet_init ]; then 
	if [ ! -z $nnet_proto ]; then
		if [ ! -f $dir/nnet/nnet.iter0 ]; then
			...
			nnet-initialize --binary=true $nnet_proto $dir/nnet/nnet.iter0 >& $dir/log/initialize_model.log || exit 1;
		fi
	fi
else
