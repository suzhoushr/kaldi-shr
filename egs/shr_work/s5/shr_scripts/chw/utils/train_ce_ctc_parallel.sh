...
max_iters=99             # max number of iterations
...
for f in $data_tr/feats.scp $data_cv/feats.scp $dir/labels.tr.gz $dir/labels.cv.gz $dir/ce_blk.tr.gz $dir/ce_blk.cv.gz; do
...
if $sort_by_len; then
  ...
  paste -d " " $data_tr/feats.scp $dir/len.tmp | sort -k3 -n - | awk '{if($3<=300) print $1 " " $2}' > $dir/train.scp || exit 1;
  cp $dir/len.tmp $dir/len.tr
  feat-to-len scp:$data_cv/feats.scp ark,t:- | awk '{print $2}' > $dir/len.tmp || exit 1;
  paste -d " " $data_cv/feats.scp $dir/len.tmp | sort -k3 -n - | awk '{if($3<=300) print $1 " " $2}' > $dir/cv.scp || exit 1;
  cp $dir/len.tmp $dir/len.cv
  rm -f $dir/len.tmp
else
...
## Set up labels  
ce_blk_tr="ark:gunzip -c $dir/ce_blk.tr.gz|"
ce_blk_cv="ark:gunzip -c $dir/ce_blk.cv.gz|"
...
if [ -z $nnet_init ]; then 
	if [ ! -z $nnet_proto ]; then
		if [ ! -f $dir/nnet/nnet.iter0 ]; then
			...
			net-initialize --binary=true $nnet_proto $dir/nnet/ce.iter0 >& $dir/log/initialize_model.log || exit 1;
		fi
	fi
else
	cp $nnet_init $dir/nnet/ce.iter0
fi

## ce pre-train
for iter in $(seq 1); do	
    train-ce-parallel --report-step=$report_step --num-sequence=$valid_num_sequence --frame-limit=$frame_num_limit \
        --learn-rate=$learn_rate --momentum=$momentum \
        --verbose=$verbose \
        "$feats_tr" "$ce_blk_tr" $dir/nnet/ce.iter$[iter-1] $dir/nnet/ce.iter${iter} \
        >& $dir/log/ce_tr.iter$iter.log || exit 1;
done
cp $dir/nnet/ce.iter1 $dir/nnet/nnet.iter0
...
# validation
    $train_tool --report-step=$report_step --num-sequence=$valid_num_sequence --frame-limit=$frame_num_limit \
        ...
        --momentum=$momentum \
        --verbose=$verbose \
        "$feats_cv" "$labels_cv" $dir/nnet/nnet.iter0 \
        >& $dir/log/cv.iter0.log || exit 1;
cvacc_prev=$(cat $dir/log/cv.iter0.log | grep "TOKEN_ACCURACY" | tail -n 1 | awk '{ acc=$3; gsub("%","",acc); print acc; }')
cur_time=`date | awk '{print $6 "-" $2 "-" $3 " " $4}'`
echo "TRAINING STARTS [$cur_time]"
echo "[NOTE] TOKEN_ACCURACY refers to token accuracy: $cvacc_prev, i.e., (1.0 - token_error_rate)."
nnet_better=$dir/nnet/nnet.iter0
for iter in $(seq $start_epoch_num $max_iters); do
    #cvacc_prev=$cvacc
    echo -n "EPOCH $iter RUNNING ... "
    nnet_next=$dir/nnet/nnet.iter${iter}
    # train
    $train_tool --report-step=$report_step --num-sequence=$num_sequence --frame-limit=$frame_num_limit \
        --learn-rate=$learn_rate --momentum=$momentum \
        --verbose=$verbose \
        "$feats_tr" "$labels_tr" $nnet_better $nnet_next \
	...
	if [ $iter -gt $halving_after_epoch ]; then
		if [ 1 == $(bc <<< "$cvacc_prev < $cvacc") ]; then
			# accepting: the loss was better, or we had fixed learn-rate, or we had fixed epoch-number,			
			cvacc_prev=$cvacc
			nnet_better=$dir/nnet/nnet.iter${iter}_lr${learn_rate}_accept
			mv $nnet_next $nnet_better
			echo "nnet accepted, and the improvement is: $rel_impr"
		else
			# rejecting,
			nnet_reject=$dir/nnet/nnet.iter${iter}_lr${learn_rate}_reject
			mv $nnet_next $nnet_reject
			echo "nnet rejected, and the improvement is: $rel_impr"
		fi
	else
		cvacc_prev=$cvacc
		nnet_better=$dir/nnet/nnet.iter${iter}_lr${learn_rate}_before
		mv $nnet_next $nnet_better
		echo "nnet before iter, and the improvement is: $rel_impr"
	fi
...
format-to-nonparallel $nnet_better $dir/final.nnet >& $dir/log/model_to_nonparal.log || exit 1;
