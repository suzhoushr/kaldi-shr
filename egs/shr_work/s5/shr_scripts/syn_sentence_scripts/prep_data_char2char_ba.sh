...
if [ $stage -le 2 ]; then
  ...
  len_cv=`expr $len_data / 50`
  len_tr=`expr $len_data - $len_cv`
  cat $data_file | head -n $len_cv > $dir/data.bob.cv.txt
  cat $data_file | tail -n $len_tr > $dir/data.bob.tr.txt
  cat $dir/data.bob.char.txt | head -n $len_cv > $dir/data.alice.cv.txt
  cat $dir/data.bob.char.txt | tail -n $len_tr > $dir/data.alice.tr.txt
