...
if [ $stage -le 2 ]; then
  cp $dir/data.alice.char.txt $dir/tmp.alice.txt
  cp $dir/data.bob.char.txt $dir/tmp.bob.txt
  cat $dir/tmp.bob.txt >> $dir/data.alice.char.txt
  cat $dir/tmp.alice.txt >> $dir/data.bob.char.txt
  rm $dir/tmp.alice.txt $dir/tmp.bob.txt
  paste $dir/data.alice.char.txt $dir/data.bob.char.txt | sort -R > $dir/alice.bob.txt
  rm $dir/data.alice.char.txt $dir/data.bob.char.txt
  cat $dir/alice.bob.txt | awk -F "\t" '{print $1}' > $dir/data.alice.char.txt
  cat $dir/alice.bob.txt | awk -F "\t" '{print $2}' > $dir/data.bob.char.txt
  rm $dir/alice.bob.txt
